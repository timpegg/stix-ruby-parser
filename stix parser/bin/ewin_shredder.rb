...
def main
  ...
  begin
    ...
    if config.webfilterrun
      @database.cull(days: config.culldays , domains: true)
      Ewinparser::Printer.print_webfilter_ticket(config.ticketfile)
    else
      ...
      @database.cull(days: config.culldays, emails: true, ips: true)
