...
class StixHeaderType < StixTypeObj
  def initialize(title=nil, description=nil, time_date_stamp=nil)
    ...
    raise "StixHeaderType wrong type: description must be a String or Nil" unless (description.is_a?(String)  | description.nil?)
    raise "StixHeaderType wrong type: time_date_stamp must be a Time or Nil" unless (time_date_stamp.is_a?(Time)  | description.nil?)
    @title = title
...
class StixObservables < StixTypeObj
  def initialize(array_of_observable, cybox_major_version=nil, cybox_minor_version=nil, cybox_update_version=nil)
    raise "StixObservables wrong type: array_of_observable must be an Array of CyboxObservable" unless (array_of_observable.is_a?(Array)  | title.nil?)
  end
end
